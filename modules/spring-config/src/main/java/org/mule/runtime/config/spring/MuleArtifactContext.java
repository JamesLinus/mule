...
public class MuleArtifactContext extends AbstractXmlApplicationContext {
  ...
  private void createApplicationModel() {
    try {
      ...
      //TODO WIP MULE-10252 looks for every module in the current classpath and caches them in the moduleExtensionStore object
      ModuleExtensionStore moduleExtensionStore = new ModuleExtensionStore();
      for (Resource springResource : artifactConfigResources) {
        Document document = xmlConfigurationDocumentLoader.loadDocument(moduleExtensionStore, springResource.getInputStream());
