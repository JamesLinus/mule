...
public class ModuleExtensionStore {
  ...
  public ModuleExtension lookupByName(final String location) {
    return extensionsByName.get(location);
  }

  public ModuleExtension lookupByNamespace(final String namespace) {
  ...
  private Document getModuleDocument(XmlConfigurationDocumentLoader xmlConfigurationDocumentLoader, Resource resource) {
    try {
      return xmlConfigurationDocumentLoader.loadDocument(this, resource.getInputStream());
    } catch (IOException e) {
  ...
  private Resource[] getModulesResources() {
    ...
    try {
      //TODO WIP MULE-10252 temporary workaround until all plugins are unified. Once done, this method (and the whole class, probably, must be erased)
